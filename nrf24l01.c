...
static uint8_t register_current_value; /*in order to change some bits of internal registers or to
										  check their content*/
...
static uint8_t current_address_width; /*current address width for receiver pipe addresses (up to 6
										 pipes), from 3 to 5 bytes*/
static uint8_t reset_flag =
	0; /*reset flag lets the software know if the nrf24l01+ has ever been reset or not*/
static uint8_t current_mode =
	DEVICE_NOT_INITIALIZED; /*current mode of operation: DEVICE_NOT_INITIALIZED, PRX, PTX, STANDBYI,
							   STANDBYII, POWER_DOWN*/
static uint8_t current_payload_width; /*payload width could be from 1 to 32 bytes, in either dynamic
										 or static forms*/
static uint8_t current_acknowledgement_state = NO_ACK_MODE;
static uint8_t dynamic_payload = DISABLE;

/*2 dimensional array of pipe addresses (5 byte address width) by default. you can change addresses
  using a new array later. Pipe 1 address could be anything. pipe 3 to 6 addresses share the first 4
  bytes with pipe 2 and only differ in byte 5*/
uint8_t datapipe_address[MAXIMUM_NUMBER_OF_DATAPIPES][ADDRESS_WIDTH_DEFAULT] = {
	{0X20, 0XC3, 0XC2, 0XC1, 0XA0}, {0X20, 0XC3, 0XC2, 0XC1, 0XA1}, {0X20, 0XC3, 0XC2, 0XC1, 0XA2},
	{0X20, 0XC3, 0XC2, 0XC1, 0XA3}, {0X20, 0XC3, 0XC2, 0XC1, 0XA4}, {0X20, 0XC3, 0XC2, 0XC1, 0XA5}};
...
{
	nrf24_read(STATUS_ADDRESS, &register_current_value, 1,
			   CLOSE); /*in order to check TX_FIFO status*/
	if((!(register_current_value & (1 << TX_FULL))) && (current_mode == PTX))
	{
		current_acknowledgement_state =
			acknowledgement_state; /*setting the acknowledgement state to either NO_ACK or ACK,
									  based on input*/
...
/*reports back transmit status: TRANSMIT_DONE, TRANSMIT_FAILED (in case of reaching maximum number
  of retransmits in auto acknowledgement mode) and TRANSMIT_IN_PROGRESS, if neither flags are set.
  automatically resets the '1' flags.*/
uint8_t nrf24_transmit_status()
{
	nrf24_read(STATUS_ADDRESS, &register_current_value, 1,
			   CLOSE); /*status register is read to check TX_DS flag*/
	if(register_current_value & (1 << TX_DS)) /*if the TX_DS == 1, */
	{
		nrf24_write(STATUS_ADDRESS, &register_current_value, 1,
					CLOSE); /*reseting the TX_DS flag. as mentioned by datasheet, writing '1' to a
							   flag resets that flag*/
		return TRANSMIT_DONE;
	}
	else if(register_current_value & (1 << MAX_RT))
	{
		nrf24_write(STATUS_ADDRESS, &register_current_value, 1,
					CLOSE); /*reseting the MAX_RT flag. as mentioned by datasheet, writing '1' to a
							   flag resets that flag*/
...
/*the receive function output is used as a polling method to check the received data inside RX
FIFOs. If there is any data available, it will be loaded inside payload array*/
uint8_t nrf24_receive(uint8_t* payload, uint8_t payload_width)
{
	if(current_mode == PRX)
	{
		...
		{
			if(dynamic_payload == DISABLE) /*if dynamic payload width is disabled, use the static
											  payload width and ignore the input*/
				payload_width = current_payload_width;
			...
			nrf24_SPI(SPI_OFF);
			nrf24_read(FIFO_STATUS_ADDRESS, &register_current_value, 1,
					   CLOSE); /*in order to check the RX_EMPTY flag*/
			if(register_current_value & (1 << RX_EMPTY)) /*if the RX FIFO is empty, reset the RX_DR
															flag inside STATUS register*/
			{
				nrf24_read(STATUS_ADDRESS, &register_current_value, 1, CLOSE);
...
/*function which uses TX_FLUSH or RX_FLUSH command to flush the fifo buffers. if successful, output
   is OPERATION_DONE. if not successful (wrong input or wrong mode of operation) output will be
   OPERATION_ERROR*/
...
void nrf24_reset()
{
	...
	nrf24_read(CONFIG_ADDRESS, &read, 1, CLOSE);
	if(read != CONFIG_REGISTER_DEFAULT)
	{
		while(1)
			;
	}
...
/*used by firmware to set the nrf24 mode in TRANSMITTER, RECEIVER, POWER_SAVING or TURN_OFF states,
  and reseting the device if it has not been done yet. This is the initializer, and everything
  starts by calling nrf24_device first.It has a higher level of abstraction than nrf24_mode and must
  be used by user*/
...
{
	nrf24_auto_acknowledgment_setup(
		datapipe); /*setting auto acknowledgment before setting dynamic payload*/
	nrf24_read(FEATURE_ADDRESS, &register_current_value, 1, CLOSE);
	if(state == ENABLE)
	{
		register_new_value =
			register_current_value |
			(1 << EN_DPL); /*EN_DPL bit turns dynamic payload width on or off on all datapipes*/
		nrf24_write(FEATURE_ADDRESS, &register_new_value, 1, CLOSE);
		if(datapipe < 7)
			register_new_value =
				(1 << datapipe) -
				1; /*turning on dynamic payload width on chosen datapipes, using DYNPD register*/
...
/*on nrf24l01+ there is only one address for PTX device which must be the same as PRX data pipe
 * address 0*/
void nrf24_datapipe_ptx(uint8_t datapipe_number)
{
	nrf24_write(TX_ADDR_ADDRESS, &datapipe_address[datapipe_number - 1][0], current_address_width,
				CLOSE);
}
...
/*datapipes are turned on and off using EN_RXADDR register, PRX datapipe addresses are located in
 * RX_ADDR_Pn, TX address is located inside TX_ADDR*/
...
void nrf24_address_width(uint8_t address_width)
{
	...
	nrf24_write(SETUP_AW_ADDRESS, &write_pointer, 1,
				CLOSE); /*5 bytes is the maximum address width available*/
	current_address_width = address_width;
}

/*datarate settings, you can choose between 2mbps, 1mbps, 250kbps*/
void nrf24_rf_datarate(uint32_t rf_datarate)
{
...
/*interrupt mask settings. 3 seperate masks for RX, TX, and RT (maximum numbers of retransmission
 * reached*/
...
/*reads the number of bytes (data_length) from the register in nrf24l01+ (address) and stores them
  inside an array (value), then closes the spi connection (spi_state = CLOSE) or leaves it open
  (spi_state = OPEN)*/
...
/*writes the number of bytes (data_length) from an array (value) inside registers in nrf24l01+
  (address), then closes the spi connection (spi_state = CLOSE) or leaves it open (spi_state =
  OPEN)*/
