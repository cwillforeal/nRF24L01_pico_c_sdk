...
#define STATIC_PAYLOAD_WIDTH_DEFAULT                                                     \
	1 /*for static payload mode, configurable between 1 and 32 bytes for PRX device ONLY \
		 (RX_PW_Pn, n for data pipe n)(no register for payload length in PTX device)*/
...
#define MASK_RX_DR                                                                                \
	6 /*mask interrupt caused by RX_DR: 1 interrupt not reflected on IRQ pin (IRQ is active low), \
		 inside CONFIG register*/
#define MASK_TX_DS                                                                                \
	5 /*mask interrupt caused by TX_DS: 1 interrupt not reflected on IRQ pin (IRQ is active low), \
		 inside CONFIG register*/
#define MASK_MAX_RT                                                                        \
	4 /*mask interrupt caused by MAX_RT means maximum number of retransmissions reached: 1 \
		 interrupt not reflected on IRQ pin (IRQ is active low), inside CONFIG register*/
#define EN_CRC \
	3 /*enale CRC, forced high if one of the bits in EN_AA is high, inside CONFIG register*/
...
#define ARD_0 4
#define ARC_3                                                                                    \
	3 /*auto retransmit count, 0000 retransmit deisabled, 1111 up to 15 retransmit on failure of \
		 AA. (inside SETUP_RETR register)*/
#define ARC_2 2
#define ARC_1 1
#define ARC_0 0
...
#define RF_DR_HIGH                                                                              \
	3 /*select between high speed data rates and works ONLY when RF_DR_LOW is 0. 0 for 1Mbps, 1 \
		 for 2Mbps*/
#define RF_PWR_1 2
#define RF_PWR_0 1
...
#define TX_FULL 0
#define PLOS_CNT_3                                                                                \
	7 /*inside OBSERVE_TX register, counts the total number of retransmissions since last channel \
		 change. reset by writing to RF_CH*/
#define PLOS_CNT_2 6
#define PLOS_CNT_1 5
#define PLOS_CNT_0 4
#define ARC_CNT_3                                                                                 \
	3 /*inside OBSERVE_TX register, counts the number of retransmissions for current transaction. \
		 reset by initiating new transaction*/
#define ARC_CNT_2 2
#define ARC_CNT_1 1
#define ARC_CNT_0 0
...
#define EN_ACK_PAY \
	1 /*used to enable auto acknowledgement with payload in PRX (inside FEATURE register)*/
#define EN_DYN_ACK 0 /**/
...
#define SETUP_AW_ADDRESS \
	0X03 /*address field length is configured in here to be 3, 4 or 5 bytes long*/
...
#define STATUS_ADDRESS \
	0X07 /*contains RX_DR, TX_DS, MAX_RT, RX_P_NO, TX_FULL, send R_REGISTER then NOP to read*/
#define OBSERVE_TX_ADDRESS                                                                         \
	0X08 /*contains ARC_CNT and PLOS_CNT, two counters for retransmission. these counters could be \
			used to assess the network quality*/
#define RPD_REG_ADDRESS 0X09
#define RX_ADDR_P0_ADDRESS                                                                    \
	0X0A /*the address for PRX device. if a packet contains this address, enhanced shockburst \
			starts validating the packet*/
#define RX_ADDR_P1_ADDRESS \
	0X0B /*a total of 6 unique addresses could be assigned to a PRX device (Multiceiver feature)*/
...
#define TX_ADDR_ADDRESS                                                                           \
	0X10 /*40 bits long register, transmit address, used for a PTX device only. configure address \
			legth in SETUP_AW register. set RX_ADDR_P0 equal to this address to handle automatic  \
			acknowledge*/
#define RX_PW_P0_ADDRESS                                                                           \
	0X11 /*these registers are for setting the static payload length in static payload length mode \
			(receiver side)*/
...
#define DYNPD_ADDRESS                                                                              \
	0X1C /*on receiver side (RX mode), this register must be set to enable dynamic payload length. \
			a PTX in dynamic mode, must have the DYNPD_P0 set*/
...
#define W_REGISTER                                                                              \
	0X20 /*write commmand and STATUS registers, 5 bit register map address, executable in POWER \
			DOWN or STANDBY modes only*/
#define R_RX_PAYLOAD                                                                             \
	0X61 /*read RX payload, 1-32 bytes. read operation starts at byte 0. payload is deleted from \
			FIFO after its read*/
...
#define REUSE_TX_PL                                                                               \
	0XE3 /*used for a PTX device, reuse last transmitted payload for an exact number. alternative \
			to auto retransmission*/
#define R_RX_PL_WID                                                                          \
	0X60 /*command for receiver side, in order to read the payload length in dynamic payload \
			length mode*/
#define W_ACK_PAYLOAD                                                                              \
	0XA0 /*used in RX mode, to write payload in TX FIFO and later transmit the payloads along with \
			ACK packet to PTX, if DPL is enabled*/
#define W_TX_PAYLOAD_NOACK                                                                         \
	0XB0 /*used in TX mode, disables AUTOACK on this specific packet. must be first enabled in     \
			FEATURE register by setting the EN_DYN_ACK bit. if used, PTX will not wait for ACK and \
			goes directly to standby I*/
